...
let uwOpponent = huskyGames2016.map(extractOpponent);
console.log(uwOpponent);
...
huskyGames2016.forEach(
  function(item) {
    if (huskiesLost(item)) {
      let opp = extractOpponent(item);
      console.log(opp + ' at UW, ' + item['opponent_score'] + ' to ' + item['home_score']);
    }
  }
);
...
let fumble = huskyGames2016.filter(function(n) {
  return n['fumbles'] >= 1;
});
...
function mostYardsPassing(g1, g2){
  if(g1['passing_yards'] == undefined) {
    return g2;
  } else if (g1['passing_yards'] > g2['passing_yards']) {
    return g1;
  } else {
    return g2;
  }
...
function makeCombinedFilter(firstCallback, secondCallback) {

  let combinedFilter = function(game) {
    return firstCallback(game) && secondCallback(game);
  };
...
let fumbledAndLostFilter = makeCombinedFilter(huskiesLost, function(n) {
  return n['fumbles'] >= 1;
});
...
let a = huskyGames2016.filter(fumbledAndLostFilter);
console.log(a);
